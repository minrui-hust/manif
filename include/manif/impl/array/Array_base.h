...
struct ArrayBase : LieGroupBase<_Derived> {
 ...
 public:
   ...
   * @return
   */
  LieGroup inverse(OptJacobianRef j_inv_m = {}) const {
    if (j_inv_m) j_inv_m->setZero();
    LieGroup inversed;
    for (auto i = 0; i < (*this).size(); ++i) {
      ElementOptJacobianRef element_jac;
      if (j_inv_m) element_jac.emplace(j_inv_m->template block<ElementDoF, ElementDoF>(i * ElementDoF, i * ElementDoF));
  ...
  Tangent log(OptJacobianRef j_t_m = {}) const {
    ...
    for (auto i = 0; i < (*this).size(); ++i) {
      ElementOptJacobianRef element_jac;
      if (j_t_m) element_jac.emplace(j_t_m->template block<ElementDoF, ElementDoF>(i * ElementDoF, i * ElementDoF));
  ...
  LieGroup compose(const LieGroupBase<_DerivedOther>& m, OptJacobianRef j_c_a = {}, OptJacobianRef j_c_b = {}) const {
    ...
    for (auto i = 0; i < (*this).size(); ++i) {
      ...
      if (j_c_a) element_jac_a.emplace(j_c_a->template block<ElementDoF, ElementDoF>(i * ElementDoF, i * ElementDoF));
      if (j_c_b) element_jac_b.emplace(j_c_b->template block<ElementDoF, ElementDoF>(i * ElementDoF, i * ElementDoF));
  ...
  Vector act(const Eigen::MatrixBase<_EigenDerived>& v,
             tl::optional<Eigen::Ref<Eigen::Matrix<Scalar, Dim, DoF>>> j_o_m = {},
    ...
    for (auto i = 0; i < (*this).size(); ++i) {
      ...
      if (j_o_m) element_j_o_m.emplace(j_o_m->template block<ElementDim, ElementDoF>(i * ElementDim, i * ElementDoF));
      if (j_o_v) element_j_o_v.emplace(j_o_v->template block<ElementDim, ElementDim>(i * ElementDim, i * ElementDim));
